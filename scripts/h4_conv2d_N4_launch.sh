#!/bin/bash
# h4_conv2d_N4_launch.sh — Step C-3: conv2d N=4 (was N=1) × 7 variants
# 4× via batch dim. Other dims unchanged: H=W=165, C=3, output-channel=3,
# kernel-height=kernel-width=7. MAX_PARALLEL=4.
...
BIN=/root/mgpusim_home/mgpusim/amd/samples/conv2d/conv2d

LAUNCH_LOG="$LOG_DIR/launch.log"
echo "[$(date '+%F %T')] === Step C-3 launch: conv2d N=4 (max=$MAX_PARALLEL) ===" > "$LAUNCH_LOG"
echo "BIN: $BIN  -N=4 -C=3 -H=165 -W=165 -output-channel=3 -kernel-height=7 -kernel-width=7" >> "$LAUNCH_LOG"
echo "" >> "$LAUNCH_LOG"
...
run_variant_bg() {
    ...
    (
        ...
        EVENT_LOG_PATH="$evt_pq" "$BIN" \
            ...
            -log2-page-size=12 \
            -N=4 -C=3 -H=165 -W=165 -output-channel=3 -kernel-height=7 -kernel-width=7 \
            $extra \
            -per-window-snapshot \
...
wait
echo "[$(date '+%F %T')] === C-3 ALL DONE ===" | tee -a "$LAUNCH_LOG"
