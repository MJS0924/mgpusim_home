#!/bin/bash
# h4_spmv_4x_launch.sh — Step C-2: spmv 4× (mid) input × 7 variants
# 4× scaling: -dim=98304 (was 65536) keeps -sparsity=0.000871 → 4× nonzeros
...
echo "[$(date '+%F %T')] === Step C-2 launch: spmv 4× (mid) (max=$MAX_PARALLEL) ===" > "$LAUNCH_LOG"
echo "BIN: $BIN  -dim=98304 -sparsity=0.000871" >> "$LAUNCH_LOG"
echo "" >> "$LAUNCH_LOG"
