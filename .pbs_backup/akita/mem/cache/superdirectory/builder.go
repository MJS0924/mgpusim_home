...
type Builder struct {
	...
	disableRSB           bool
	disableCBF           bool
	parallelBankSearch   bool // true이면 dirLatency 한 번에 모든 candidate bank를 동시 lookup
...
// WithParallelBankSearch toggles between sequential (false, default) and
// one-shot parallel (true) directory bank search. In parallel mode, a single
// directory pipeline traversal looks up every candidate bank and returns the
// first hit; on full miss it falls through to doWriteMiss without an extra
// pipeline cycle per bank.
func (b Builder) WithParallelBankSearch(v bool) Builder {
	b.parallelBankSearch = v
	return b
}
...
func (b *Builder) configureCache(cacheModule *Comp) {
	...
	cacheModule.parallelBankSearch = b.parallelBankSearch
